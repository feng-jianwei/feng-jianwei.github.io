...
eBPF 挂载点（probe/attachment types）的**性能比较**是性能分析中最核心的话题之一，尤其在实时系统、中间件等高频/低延迟场景下，选错类型可能导致系统整体延迟上升 10–50%+，甚至触发 soft lockup 或测量失真。

下面基于内核源码机制、社区 benchmark（Cloudflare ebpf_exporter、Alexei Starovoitov 原始提交、LWN 文章等）和生产工具实测（bpftrace、Tracee、Tetragon、Falco 等），给出 主流内核的**单次触发 overhead 排名**（ns 级，热路径下，JIT 启用，x86-64/arm64 类似）。

**关键说明**：

- **性能度量**：单次触发/执行 overhead (ns)，热路径下（高频，如每秒百万级）。数据来源于 iximiuz 2025 测试（fentry 最快）、Cloudflare benchmark（tracing: fentry > raw_tp > kprobe）、Cilium XDP 测试（~10 ns/packet）、LWN/Alexei 原始提交（raw_tp 比 tp 快 10–30%）。
...
## 2. Networking/流量专用类（焦点：per-packet 处理开销，早路径 = 低开销）

这些是网络栈 inline 程序，非“probe”而是“处理器/过滤器”。开销以 ns/packet 计算（高频包下累积 ~1–5% 系统 CPU）。fentry 可 attach 以 tracing 它们（额外 20–80 ns）。基于 Cilium 测试（XDP 最快）、iximiuz 2025 benchmark（XDP driver mode 优于 skb）。
...
## 3. Other/杂类（非 tracing/non-networking，性能视具体钩子）
...
- **fentry/fexit** 是当前 tracing overhead 的**天花板**（trampoline + direct call，几乎零额外异常），在热点函数上比 kprobe 快 **5–10 倍**，比 raw_tp 快 **1.5–3 倍**（Cloudflare benchmark、iximiuz 2025 测试一致）。
...
### 真实 benchmark 摘录（部分来源）

- **Alexei raw_tp 引入时**（LWN 2018，至今经典）：raw_tp 在高频 syscall 上比 tracepoint 快 ~20%，比 kprobe 明显快。
- **Cloudflare ebpf_exporter**（持续维护到 2025+）：tracepoint > fentry > kprobe（单核 getpid 测试，fentry 最接近零开销）。
- **Cycode/Cimon 2025 优化**：raw_tp > tracepoint > fentry > kprobe > uprobe（但 fentry 在函数级热点上反超 raw_tp）。
- **iximiuz Labs / eBPFChirp 2025**：fprobes (fentry) 远低于 kprobe，尤其热函数；raw_tp 略优于普通 tp。
...
# eBPF 性能分析完整流程（纯 C++ 技术栈 + libbpf + BPF Skeleton）
...
- **步骤：**
  ...
  - 创建文件 latency_tracer.bpf.c（eBPF C 代码）。
    ...
    #include <vmlinux.h>  // 通过 bpftool 生成，包含所有内核类型
    #include <bpf/bpf_helpers.h>
    #include <bpf/bpf_tracing.h>
    
    // Map: 存储线程开始时间 (key: tid, value: ns)
    struct {
    ...
    // Map: 延迟 histogram (log2 桶，便于 P99 分析)
    struct {
        __uint(type, BPF_MAP_TYPE_HASH);
        __uint(max_entries, 64);  // 桶数
        __type(key, u64);         // 桶索引 (log2(delta))
        __type(value, u64);       // 计数
    } latency_hist SEC(".maps");
    
    // 入口探针: 记录时间
    SEC("usdt/myapp:request_start")
    int BPF_USDT(trace_request_start, int arg) {
        u32 tid = bpf_get_current_pid_tgid() >> 32;  // 线程 ID
        u64 ts = bpf_ktime_get_ns();
        bpf_map_update_elem(&start_times, &tid, &ts, BPF_ANY);
        bpf_printk("Start: arg=%d", arg);  // 可选调试
        return 0;
    }
    
    // 出口探针: 计算延迟，更新 histogram
    SEC("usdt/myapp:request_end")
    int BPF_USDT(trace_request_end) {
        u32 tid = bpf_get_current_pid_tgid() >> 32;
        u64 *start = bpf_map_lookup_elem(&start_times, &tid);
        if (start) {
            u64 delta = bpf_ktime_get_ns() - *start;  // ns 级延迟
            u64 bucket = bpf_log2l(delta);            // log2 桶
            u64 *count = bpf_map_lookup_elem(&latency_hist, &bucket);
            if (count) {
                __sync_fetch_and_add(count, 1);
            } else {
                u64 one = 1;
                bpf_map_update_elem(&latency_hist, &bucket, &one, BPF_ANY);
            }
...
- **步骤：**
  ...
  - 创建用户态 C++ 文件 tracer.cpp（你的 daemon 或工具）。
    ...
    #include "latency_tracer.skel.h"  // skeleton 头
    ...
    int main(int argc, char **argv) {
        ...
        int pid = std::stoi(argv[1]);  // 应用 PID
    
        // 打开、加载 eBPF
        struct latency_tracer_bpf *skel = latency_tracer_bpf__open();
        if (!skel) {
            std::cerr << "Failed to open BPF skeleton\n";
            return 1;
        }
    
        // 设置 USDT 附加（libbpf 自动处理）
        bpf_usdt_spec usdt_specs[2] = {};  // 对于两个探针
        // 注意：libbpf 需要手动指定 USDT 路径或用 bpf_program__attach_usdt
        // 简化版：用 libbpf 的 usdt manager（需 libbpf 1.0+）
        skel->links.trace_request_start = bpf_program__attach_usdt(skel->progs.trace_request_start, pid, "./your_app", "myapp", "request_start");
        skel->links.trace_request_end = bpf_program__attach_usdt(skel->progs.trace_request_end, pid, "./your_app", "myapp", "request_end");
    
        if (!skel->links.trace_request_start || !skel->links.trace_request_end) {
            std::cerr << "Failed to attach USDT\n";
            latency_tracer_bpf__destroy(skel);
            return 1;
        }
    
        if (latency_tracer_bpf__load(skel)) {
            std::cerr << "Failed to load BPF\n";
            latency_tracer_bpf__destroy(skel);
            return 1;
        }
    
        std::cout << "SDT enabled, tracing PID " << pid << "\n";
...
- **步骤：** 在 tracer.cpp 的 main 中添加循环读取。
  ...
  while (true) {
      ...
      for (u64 bucket = 0; bucket < 64; ++bucket) {
          u64 *count;
          if (bpf_map_lookup_elem(bpf_map__fd(skel->maps.latency_hist), &bucket, &count) == 0) {
              std::cout << "Bucket " << bucket << " (" << (1ULL << bucket) << " ns): " << *count << "\n";
          }
      }
      // 计算百分位（伪码：排序 counts，找 P99）
      // ... 自定义分析 ...
      sleep(5);  // 每5s 打印一次
  }
  ```
